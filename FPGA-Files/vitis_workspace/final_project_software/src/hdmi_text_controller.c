...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	hdmi_ctrl->COLOR_PALETTE[address] = (green << 4) + blue;
	hdmi_ctrl->COLOR_PALETTE[address + 1] = red;
	printf(" %d, %d, %d", red, green, blue);
	printf("\n");
	printf(" We have %d, and value %x%x", color, hdmi_ctrl->COLOR_PALETTE[PALETTE_START + (color*2 + 1)], hdmi_ctrl->COLOR_PALETTE[PALETTE_START + (color*2)]);
	printf("\n");
}
